...
class politics {
    ...
    private func fetchNewsAPI() async throws -> [Article] {
        let url = URL(string: "https://newsapi.org/v2/top-headlines?sources=politico,the-guardian-uk,sky-news&apiKey=\(newsApiKey)")!
    ...
    private func fetchNYT() async throws -> [Article] {
        let url = URL(string: "https://api.nytimes.com/svc/topstories/v2/politics.json?api-key=\(nytApiKey)")!
        let (data, _) = try await URLSession.shared.data(from: url)
        let json = try JSONSerialization.jsonObject(with: data) as! [String: Any]
    ...
    private func fetchGuardian() async throws -> [Article] {
        let url = URL(string: "https://content.guardianapis.com/politics?order-by=newest&show-fields=thumbnail,trailText,webPublicationDate&api-key=\(guardianApiKey)")!
        let (data, _) = try await URLSession.shared.data(from: url)
        let json = try JSONSerialization.jsonObject(with: data) as! [String: Any]
